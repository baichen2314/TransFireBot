...
namespace SysBot.Pokemon
{
    public class PokeTradeLogNotifier<T> : IPokeTradeNotifier<T> where T : PKM, new()
    {
        ...
        {
            LogUtil.LogInfo($"开始{info.Trainer.TrainerName}的交换循环, 发送{GameInfo.GetStrings("zh-Hans").Species[info.TradeData.Species]}", routine.Connection.Label);
        }

        public async Task TradeSearching(PokeRoutineExecutor<T> routine, PokeTradeDetail<T> info)
        {
            LogUtil.LogInfo($"搜索与 {info.Trainer.TrainerName}的交换, 发送 {GameInfo.GetStrings("zh-Hans").Species[info.TradeData.Species]}", routine.Connection.Label);
        }
        ...
        {
            LogUtil.LogInfo($"完成了{info.Trainer.TrainerName} {GameInfo.GetStrings("zh-Hans").Species[info.TradeData.Species]}的{GameInfo.GetStrings("zh-Hans").Species[result.Species]}交换", routine.Connection.Label);
            OnFinish?.Invoke(routine);
        }
        ...
        {
            LogUtil.LogInfo($"通知{info.Trainer.TrainerName}关于他们的{GameInfo.GetStrings("zh-Hans").Species[result.Species]}", routine.Connection.Label);
            LogUtil.LogInfo(message, routine.Connection.Label);
        }
